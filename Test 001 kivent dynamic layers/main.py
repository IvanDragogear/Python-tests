...
class TestGame(Widget):
    ...
    def draw_some_stuff(self):
        ...
        self.ent_layers.append([sp.player,'layer_0'])
        ...
        for i in range(9):
            ...
            self.ent_layers.append([ent,layer])
        self.order_layers()
    ...
    def order_layers(self,*args):
        ...
        a = []
        b = []
        for ent, layer in self.ent_layers:
            a.append([ent.position.y,layer])
        a = sorted(a, key=itemgetter(0))
        for layer in a:
            b.append(layer)
        if not b == camera.render_system_order:
            camera.render_system_order = b
            camera.clear_widgets()
            for pos, layer in reversed(a):
                camera.add_widget(self.layers[layer])
        Clock.schedule_once(self.order_layers,0.3)
